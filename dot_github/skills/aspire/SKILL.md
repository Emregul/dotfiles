...
**If `aspire resource <name> rebuild` gets stuck** in a "Waiting" state, fall back to a full `aspire start` which auto-stops the previous instance. This is more reliable for resources with complex dependencies (e.g., Orleans silos that other services wait on).

### Debugging issues

Before making code changes, inspect the app state:

1. `aspire describe` — check resource status (look for Unhealthy/Failed resources)
2. `aspire otel logs <resource>` — view structured logs
3. `aspire logs <resource>` — view console output (useful for startup crashes)
4. `aspire otel traces <resource>` — view distributed traces
5. Check the Aspire dashboard (URL shown by `aspire describe`) for a visual overview

**Cross-service connectivity issues**: If services can't talk to each other, check traces for broken parent-child span relationships. Common causes:
- Orleans silos advertising the wrong IP address (e.g., VPN interface instead of loopback) — check `AdvertisedIPAddress` in silo config
- Resources starting before their dependencies are healthy — verify `WaitFor()` relationships in the AppHost

**EventHub emulator transient errors**: On restart, EventHub emulators often log errors about stale blob container references and offset conflicts. These are transient and self-resolve once the emulators fully initialize. Don't chase these unless they persist for more than ~60 seconds.

### Verifying traces end-to-end

After making tracing changes, verify the full trace pipeline:

1. Start the app: `aspire start`
2. Send a test request to the API endpoint (use `curl` or the test API file)
3. `aspire otel traces <resource>` — look for spans with correct parent-child relationships across service boundaries
4. Confirm trace context propagates: FrontDoor spans should be parents of Orleans grain spans, which should be parents of downstream (e.g., Redis) spans
5. Check for error recording: trigger an error path and verify spans have error status and exception events
...
### Orleans IP binding on macOS
Orleans 10.x changed its IP auto-detection behavior. On macOS with VPN or tunnel interfaces (e.g., `10.10.10.10` loopback alias), Orleans silos may advertise the wrong IP. Other services read this from clustering storage and fail to connect. Fix: set `AdvertisedIPAddress = IPAddress.Loopback` in development configuration. This does not affect production (which uses real IPs).

### Container networking differences (macOS vs WSL/Windows)
Aspire containers may behave differently on macOS Docker Desktop vs WSL2. If containers can't communicate on macOS but worked on Windows/WSL, check:
- Docker Desktop networking mode (host vs bridge)
- Service discovery addresses (localhost vs container names)
- VPN/tunnel interfaces that Docker may route through
